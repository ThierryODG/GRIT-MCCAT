<x-guest-layout>
    ...
    <div class="text-center mb-8">
        ...
        <div class="mb-6">
            <img src="{{ asset('images/logo-mccat-300x300.jpg') }}" alt="SIGR-ITS" class="mx-auto h-16">
        </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox"
                    class="rounded border-gray-300 text-gray-600 focus:ring-gray-500 transition duration-150"
                    name="remember">
                <span class="ms-2 text-sm text-gray-600">{{ __('Se souvenir de moi') }}</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm text-gray-600 hover:text-gray-800 transition duration-150 font-medium"
        ...
        <div class="pt-4">
            <x-primary-button
                class="w-full justify-center bg-gray-800 hover:bg-gray-900 focus:bg-gray-900 active:bg-gray-950 text-white font-medium py-3 transition-colors duration-200">
                {{ __('Se connecter') }}
            </x-primary-button>
        </div>
    </form>
