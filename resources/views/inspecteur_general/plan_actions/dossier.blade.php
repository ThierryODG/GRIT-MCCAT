...
@section('content')
                ...
                <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900">{{ $recommandation->pointFocal->name ?? 'Non assigné' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-500">Responsable</label>
                            <p class="text-gray-900">{{ $recommandation->responsable->name ?? 'Non assigné' }}</p>
                        </div>
                        <div>
        ...
        ($recommandation->priorite == 'moyenne' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
                            ...
                            @foreach($recommandation->plansAction as $plan)
                                <div class="p-4 border border-gray-200 rounded-lg bg-purple-50">
                                    ...
                                    <div class="text-sm text-gray-600">
                                        ...
                                        <p><strong>Validé responsable le :</strong>
                                            {{ $plan->date_validation_responsable ? $plan->date_validation_responsable->format('d/m/Y à H:i') : 'N/A' }}
                                        </p>
                                    </div>
                                </div>
                            @endforeach
            ...
            <div class="space-y-6">
                ...
                <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <!-- Valider -->
                    <form method="POST"
                        action="{{ route('inspecteur_general.plan_actions.recommandation_valider', $recommandation) }}"
                        class="mb-4">
                        @csrf
                        <div class="mb-4">
                        ...
                        <button type="button" onclick="openConfirmModal('valider')"
                            class="w-full px-4 py-2 text-white transition bg-green-600 rounded-lg hover:bg-green-700">
                    ...
                    <!-- Rejeter -->
                    <form method="POST"
                        action="{{ route('inspecteur_general.plan_actions.recommandation_rejeter', $recommandation) }}"
                        class="mb-4">
                        @csrf
                        <div class="mb-4">
                            ...
                            <textarea name="motif" id="motif" rows="3"
                                ...
                                placeholder="Expliquez pourquoi ce plan n'est pas acceptable..." required></textarea>
                        </div>
                        <button type="button" onclick="openConfirmModal('rejeter')"
                            class="w-full px-4 py-2 text-white transition bg-red-600 rounded-lg hover:bg-red-700">
    ...
    <div id="confirmModal"
        class="fixed inset-0 z-50 flex items-center justify-center transition-opacity bg-black bg-opacity-50 opacity-0"
        style="display: none;">
        <div class="max-w-md mx-4 bg-white rounded-lg shadow-xl">
            ...
            <div class="flex justify-end gap-3 p-6 border-t border-gray-200">
                ...
                <button type="button" id="confirmBtn"
                    class="px-4 py-2 text-white transition bg-blue-600 rounded-lg hover:bg-blue-700" onclick="submitForm()">
                    Confirmer
                </button>
            </div>
        </div>
