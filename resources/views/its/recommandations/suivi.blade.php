                ...
                @foreach($recommandation->plansAction as $index => $action)
                    <div x-show="currentStep === {{ $index }}" class="p-8 flex-1 flex flex-col h-full overflow-y-auto">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-lg border border-gray-100">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-700">
                                        {{ $action->date_fin_prevue ? \Carbon\Carbon::parse($action->date_fin_prevue)->format('d/m/Y') : 'Non définie' }}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-auto border-t border-gray-100 pt-8">
                            ...
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Commentaire d'exécution (Point
                                        Focal)</label>
                                    <div
                                        class="w-full p-4 bg-gray-50 rounded-lg border border-gray-200 text-gray-700 italic min-h-[100px]">
                                        ...
                                        <ul class="space-y-2">
                                            @foreach($action->preuvesExecution as $preuve)
                                                <li class="flex items-center text-sm">
                                                    ...
                                                    <a href="{{ route('point_focal.avancement.download_preuve', $preuve) }}"
                                                        class="text-blue-600 hover:underline" target="_blank">
                                                        {{ $preuve->file_name }}
                                                    </a>
                                                    <span
                                                        class="text-xs text-gray-400 ml-2">({{ $preuve->created_at->format('d/m/Y') }})</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @else
                                ...
                                <div class="flex items-center gap-4 pt-4">
                                    ...
                                    @if($action->statut_execution === 'termine')
                                        <span
                                            class="px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">Terminé</span>
                                    @elseif($action->statut_execution === 'en_cours')
                                        <span class="px-3 py-1 rounded-full text-sm font-medium bg-orange-100 text-orange-800">En
                                            cours</span>
                                    @else
                                        <span class="px-3 py-1 rounded-full text-sm font-medium bg-gray-100 text-gray-800">Non
                                            démarré</span>
                                    @endif
                                </div>
                            </div>
