    ...
    <div class="container px-4 py-6 mx-auto">
        ...
        <div class="mb-8 flex justify-between items-end">
            ...
            <div class="flex bg-gray-200 rounded-lg p-1">
                <a href="{{ route('point_focal.recommandations.index', array_merge(request()->query(), ['view' => 'active'])) }}"
                ...
                <a href="{{ route('point_focal.recommandations.index', array_merge(request()->query(), ['view' => 'archives'])) }}"
                    class="px-3 py-1 text-sm rounded-md transition {{ request('view') === 'archives' ? 'bg-white shadow text-gray-800 font-medium' : 'text-gray-600 hover:text-gray-800' }}">
                    Archives
                </a>
            </div>
                                ...
                                @foreach($its->recommandations as $recommandation)
                                    <tr class="transition-colors hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span
                                                class="font-mono text-sm font-medium text-gray-900">{{ $recommandation->reference }}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-medium text-gray-900">
                                                {{ Str::limit($recommandation->titre, 60) }}</div>
                                            <div class="mt-1 text-sm text-gray-500">
                                                {{ Str::limit($recommandation->description, 80) }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border {{ $prioriteColors[$recommandation->priorite] ?? 'bg-gray-100 text-gray-800' }}">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $statutColors[$recommandation->statut] ?? 'bg-gray-100 text-gray-800' }}">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            <div class="flex items-center">
                                                ...
                                                @if($recommandation->estEnRetard())
                                                    <span
                                                        class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 border border-red-200">
                                                        🚨 Retard
                                                    </span>
                                                @endif
                                            </div>
